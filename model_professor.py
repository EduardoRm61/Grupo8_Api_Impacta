...
professores = {"professor": [
    ...
    "obs": "Contato com aluno via Chat"},    # ok
...
def apaga_tudo():
    professores["Professor"] = []   # não tinha posto - apaguei listas de turma e alunos -deixando apenas professor já que não as uso aqui
...
def ProfessorExistente(Id_professor):
    for professor in professores["professor"]:
        if professor["id"] == Id_professor: 
            return True  
    return False                   #havia tirado - coloquei de novo - ok

def procurarProfessorPorId(id_professor):   #def é minúscula
    for professor in professores["professor"]:
        if professor['id'] == id_professor:
            return professor
    raise ProfessorNaoIdentificado()  #ok
...
def resetar_professores():
    professores["professor"] = []
    return                            #acho que não tinha posto - ok